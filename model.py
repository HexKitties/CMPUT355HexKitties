...
class HexModel():
    ...
    def place_chess2(self, chess_pos):
        board = [self]
        board[0].board[chess_pos[0]][chess_pos[1]] = board[0].player_turn
        board[0].player_turn = (board[0].player_turn + 1) % 2
        board[0].history.append(chess_pos)
        return board[0]
    ...
    def get_winner(self):
        ...
        while len(Q) > 0:
            ...
            if c in set2:
                return 1
            for d in self.nbrs[c]:
                if self.board[d[0]][d[1]] == 1 and d not in seen:
                    Q.append(d)
                    seen.add(d)
        return 2
